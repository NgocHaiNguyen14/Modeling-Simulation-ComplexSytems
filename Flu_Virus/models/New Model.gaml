...
* Description: Model of flu virus spread with family structures, vaccination, variants and 30-day limit
*/
...
global {
    ...
    float base_infection_probability <- 0.5;
    int isolation_duration <- 12;
    float test_percentage <- 0.001;
    
    // Vaccination parameters
    float vaccination_rate <- 0.0005;
    ...
    bool should_end <- false update: current_date >= simulation_start_date + 30 #day;
    ...
    date simulation_start_date <- current_date;
    
    // Helper action for variant cases
    map<string, int> get_variant_cases {
        map<string, int> cases;
        loop variant over: existing_variants {
            cases[variant] <- length(person where (each.current_variant = variant and each.is_infected));
        }
        return cases;
    }
    ...
    action generate_new_variant (string parent_variant) type: string {
        ...
        float mutation_factor <- 0.5 + rnd(1.0);
    ...
    init {
        ...
        ask (length(person) * 0.1) among person {
            is_infected <- true;
    ...
    reflex end_simulation when: should_end {
        do pause;
    }
    
    reflex virus_mutation when: every(24 #hour) {
        ask person where (each.is_infected and each.current_variant != nil) {
    ...
    reflex update_vaccine_target when: every(24 #hour) {
        map<string, int> current_cases <- get_variant_cases();
        if (!empty(current_cases)) {
            string most_prevalent_variant <- existing_variants with_max_of(current_cases[each]);
            if (most_prevalent_variant != nil and most_prevalent_variant != current_vaccine_target) {
...
experiment flu_simulation type: gui {
    ...
    output {
        ...
        display variant_charts refresh: every(1 #cycle) {
            chart "All Variant Cases" type: series position: {0, 0} size: {1.0, 0.5} {
                data "Original" value: length(person where (each.is_infected and each.current_variant = "original")) 
                     color: #red style: line;
                
                loop i from: 1 to: variant_counter {
                    string variant_name <- "variant_" + i;
                    data variant_name 
                         value: length(person where (each.is_infected and each.current_variant = variant_name))
                         color: rgb((i * 50) mod 255, (i * 100) mod 255, (i * 150) mod 255)
                         style: line;
                }
            }
            
            chart "Top 5 Active Variants" type: pie position: {0, 0.5} size: {0.5, 0.5} {
                loop variant over: existing_variants {
                    int cases <- length(person where (each.is_infected and each.current_variant = variant));
                    if (cases > 0) {
                        data variant value: cases;
                    }
                }
            }
        }
        
        display population_status refresh: every(1 #cycle) {
            chart "Population Status" type: series position: {0, 0} size: {1.0, 1.0} {
        ...
        // Detailed monitors
        monitor "Simulation Day" value: int((current_date - simulation_start_date)/#day);
        monitor "Total Population" value: length(person);
        monitor "Total Infected" value: person count (each.is_infected);
        ...
        monitor "Variant Case Details" value: variant_cases;
        monitor "Most Prevalent Variant" 
            value: empty(variant_cases) ? "None" : 
                  existing_variants with_max_of(variant_cases[each]);
    }
}